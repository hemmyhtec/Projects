<template>
  <div class="card">
    ...
    <div class="text-center">
      ...
      <div class="text-body1">@ {{ profiles.profession }}</div>
      ...
      <div class="row">
        ...
        <div class="col">
          ...
          <select v-model="ava">
            <!-- <label for="">Set Availability</label> -->
              <option v-for="available in availables" :key="available"> {{ available }} </option>
          </select>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      profiles: {},
      ava: "Set Availability",
      small: ref(false),
      availables: ["Available", "Not Available"]
    };
  },
  methods: {
    getProfilePhoto() {
      if(this.profiles.profilepicture){
        // this.profiles.file = this.profiles.file.replace(/^\/+|\/+$/g, '')
        return (`http://localhost:8081/images/${this.profiles.profilepicture}`)
      } else {
        console.log('Logging error loading image from Folder', this.profiles.profilepicture)
      }
  },
  ...
  async mounted() {
    ...
    // console.log('Logging image from Axios get Api',this.profiles.file)
    // const image = this.profiles.file
  },
});
</script>
<style lang="sass">
